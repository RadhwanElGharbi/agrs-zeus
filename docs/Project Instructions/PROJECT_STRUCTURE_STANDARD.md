...
- **IMPORTANT**: Do NOT nest project folders (e.g., avoid `PROJECT_NAME/PROJECT_NAME/`)
...
├── pipeline_specs.json            # Pipeline physical specifications
├── aoi/                           # Area of Interest definition
│   ├── aoi.{geojson|gpkg|kml|kmz} # Primary AOI geometry (any supported format)
...
│   │   │   └── ...
│   │   └── processed/             # CANONICAL: Reprojected, clipped rasters
...
│       │   └── ...
│       └── processed/             # CANONICAL: Reprojected vectors
│           ├── osm_roads_epsg32633_processed.gpkg
│           ├── osm_roads_epsg32633_processed.gpkg.json
│           └── ...
├── inputs/                        # Client-provided materials
│   ├── *.pdf                      # Engineering drawings, specs
│   ├── *.xlsx                     # Routing criteria, cost matrices
│   └── README.md                  # Index of input materials
├── scripts/                       # Project-specific scripts
│   ├── fetch_*.sh                 # Data fetching scripts
...
### Root-Level Script Policy

**IMPORTANT**: All project scripts (`.py`, `.sh`) MUST be placed in appropriate directories:
- **`scripts/`**: General project scripts (fetching, processing, validation)
- **`PIRL/`**: PIRL-specific training and analysis scripts

Do NOT place scripts at the project root level. This keeps the project structure clean and organized.
...
All formats are supported and will produce the same result: AOI, start/end points mapped and displayed with all standard information in the directory and GUI.

**AOI Metadata Requirements** (`aoi/project_aoi.json`):

This file is **auto-populated** from the AOI file when the project is created or when the AOI is processed. The following fields are automatically extracted:
- `aoi_area_km2` - Calculated from the AOI geometry extent
- `aoi_countries` - Inferred from the AOI location using reverse geocoding

```json
{
  "aoi_file": "aoi/aoi.gpkg",
  "aoi_area_km2": 1505.19,
  "aoi_countries": ["Italy"],
  "crs_epsg": 32633,
  "crs_name": "WGS 84 / UTM zone 33N",
  "start_point": {
    "file": "aoi/start_point.kml",
    "latitude": 43.388493,
    "longitude": 13.514053
  },
  "end_point": {
    "file": "aoi/end_point.kml",
    "latitude": 42.898254,
    "longitude": 13.877811
  }
}
...
**Note**: `aoi_metadata.json` is **DEPRECATED**. Use `project_aoi.json` instead.
...
Once AOI and CRS are defined, **all relevant datasets must be fetched immediately** using available CLI tools or the GUI Dataset Fetching interface.
...
zeus tools osm_roads_fetch --aoi aoi/aoi.geojson -o data/vectors/raw/osm_roads_raw.gpkg
zeus tools osm_railways_fetch --aoi aoi/aoi.geojson -o data/vectors/raw/osm_railways_raw.gpkg
zeus tools osm_waterways_fetch --aoi aoi/aoi.geojson -o data/vectors/raw/osm_waterways_raw.gpkg
zeus tools gadm_fetch --country ITA --aoi aoi/aoi.geojson -o data/vectors/raw/boundaries_raw.gpkg
zeus tools tinitaly_fetch --aoi aoi/aoi.geojson -o data/rasters/raw/dem_tinitaly_10m_raw.tif
zeus tools esa_worldcover_fetch --aoi aoi/aoi.geojson -o data/rasters/raw/landcover_esa_worldcover_raw.tif
# ... (continue for all relevant datasets)
```

All fetch operations must be logged to `logs/fetch.log` or `logs/dataset_fetch.log`.
...
- **Processed Data (CANONICAL):** All reprojected, clipped datasets MUST be stored in `processed/` subdirectories
...
**IMPORTANT**: Symlinks at the `data/rasters/` and `data/vectors/` root level are **DEPRECATED** as of v2.0.

Previous requirement (no longer needed):
```
data/rasters/dem.tif -> processed/dem_epsg32633_processed.tif  # DEPRECATED
data/vectors/roads.gpkg -> processed/osm_roads_epsg32633_processed.gpkg  # DEPRECATED
```

The GUI now reads directly from `processed/` directories. Existing symlinks can be removed.

#### File Naming Conventions
- **Raw files:** `{dataset_name}_{source}_{resolution}_raw.{ext}`
  - Example: `dem_tinitaly_10m_raw.tif`
  - Example: `osm_roads_raw.gpkg`
- **Processed files:** `{category}_epsg{code}_processed.{ext}`
  - Example: `dem_epsg32633_processed.tif`
  - Example: `roads_epsg32633_processed.gpkg`
...
- `logs/fetch.log` - Data acquisition operations (CLI)
- `logs/dataset_fetch.log` - Data acquisition operations (GUI)
...
4. Create `project_metadata.json` and `aoi/project_aoi.json`
5. Initialize log files
6. Create `inputs/` directory and add client-provided materials
...
2. Fetch all datasets using CLI tools or GUI
3. Download any datasets requiring manual acquisition
4. Store raw data in `data/rasters/raw/` and `data/vectors/raw/`
...
- [ ] `aoi/project_aoi.json` exists and is complete
- [ ] AOI file exists in at least one format
- [ ] Project CRS is documented and appropriate (projected, not geographic)
- [ ] Measurement units are documented (default: SI)
- [ ] `inputs/` directory exists with client materials indexed
...
#### Step 5: Cost Matrix (MANDATORY for PIRL training)
- [ ] `docs/cost_matrix/` directory exists
- [ ] `COST_MATRIX_COMPLETE.csv` created with all cost categories
- [ ] `COST_MATRIX_COMPLETE.xlsx` created (optional, for editing)
- [ ] `COST_MATRIX_README.md` documents all cost assumptions

---

## Migration Guide for Existing Projects

Projects created before v2.0 should be updated to comply with the new standard:

### Required Changes

1. **Move root-level scripts to `scripts/` or `PIRL/`**
   ```bash
   cd /opt/agrs/Projects/<PROJECT_NAME>
   mkdir -p scripts
   mv *.py *.sh scripts/ 2>/dev/null || true
   # Move PIRL-specific scripts to PIRL/
   mv scripts/train_*.sh scripts/generate_route*.py PIRL/ 2>/dev/null || true
   ```

2. **Remove symlinks from `data/rasters/` and `data/vectors/`**
   ```bash
   find data/rasters -maxdepth 1 -type l -delete
   find data/vectors -maxdepth 1 -type l -delete
   ```

3. **Create `project_aoi.json` if only `aoi_metadata.json` exists**
   ```bash
   # Rename if exists
   mv aoi/aoi_metadata.json aoi/project_aoi.json 2>/dev/null || true
   # Add missing fields (aoi_area_km2, aoi_countries) manually or via script
   ```

4. **Add `docs/cost_matrix/` if PIRL training is planned**
   ```bash
   mkdir -p docs/cost_matrix
   # Copy template or create cost matrix files
   ```

5. **Update `project_metadata.json` to use nested `crs` object**
   ```bash
   # Manual update required - convert:
   # "crs_epsg": 32633, "crs_name": "..." 
   # to:
   # "crs": { "epsg": 32633, "name": "..." }
   ```

6. **Add `country` and `iso3` fields to `project_metadata.json`**
   ```json
   {
     "country": "Italy",
     "iso3": "ITA"
