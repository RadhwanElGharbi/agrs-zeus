    ...
    try {
        if (toolName == "dem_fetch") {
            return tools_dem_fetch(
                ...
                params.value("provider", "auto").toString().toStdString(),
                params.value("to_crs", "EPSG:4326").toString().toStdString(),
                params.value("align_to", "").toString().toStdString(),
                params.value("output").toString().toStdString(),
                params.value("overwrite", false).toBool(),
                params.value("dry_run", false).toBool()
            );
        } else if (toolName == "terrain_slope") {
            return tools_terrain_slope(
                params.value("input").toString().toStdString(),
                params.value("output").toString().toStdString(),
                params.value("as_percent", false).toBool(),
                params.value("compute_edges", true).toBool(),
                params.value("algorithm", "Horn").toString().toStdString(),
                params.value("overwrite", false).toBool()
            );
        } else if (toolName == "raster_calc") {
            std::vector<std::string> inputs;
            if (params.contains("inputs")) {
                QStringList inputList = params.value("inputs").toStringList();
            ...
            return tools_raster_calc(
                ...
                params.value("data_type", "Float32").toString().toStdString(),
                params.value("overwrite", false).toBool()
            );
        } else if (toolName == "vector_buffer") {
            return tools_vector_buffer(
                ...
                params.value("segments", 8).toInt(),
                params.value("end_cap_style", "round").toString().toStdString(),
                params.value("dissolve", false).toBool(),
                params.value("overwrite", false).toBool()
            );
        } else if (toolName == "pirl_create_config") {
            return tools_pirl_create_config(
                ...
                params.value("output").toString().toStdString(),
                params.value("interactive", false).toBool()
            );
        } else if (toolName == "analyze_fetch_tools") {
