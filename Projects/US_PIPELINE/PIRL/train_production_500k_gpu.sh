#!/bin/bash
################################################################################
# US_PIPELINE PIRL - Production Training (500K timesteps) - GPU VERSION
#
# Full production training run for deployment-ready model.
# Optimized for CUDA GPU acceleration.
#
# Expected runtime (GPU, 24 envs, batch 2048): ~10-20 minutes
#
# Success criteria:
#   - Goal reach rate > 80%
...
echo -e "${BLUE}=====================================================${NC}"
echo -e "${BLUE}US_PIPELINE PIRL - Production Training 500K (GPU)${NC}"
echo -e "${BLUE}=====================================================${NC}"
...
if ! command -v nvidia-smi &> /dev/null; then
    ...
    echo "   Use train_production_500k_cpu.sh instead."
    exit 1
fi
...
echo -e "${YELLOW}⏱️  Estimated runtime: 10-20 minutes${NC}"
echo ""
echo -e "${GREEN}Starting GPU production training...${NC}"
echo "  Log file: $LOG_FILE"
echo ""

# Prompt for confirmation
read -p "Continue with 500K timestep GPU training? (y/N): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}🚀 GPU TRAINING STARTED${NC}"
echo -e "${BLUE}========================================${NC}"
...
if [ $EXIT_CODE -eq 0 ]; then
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}✅ GPU PRODUCTION TRAINING COMPLETE!${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo ""
    ...
    echo "  - Out of VRAM: Reduce batch_size (2048 → 1024)"
    echo "  - CUDA errors: Check nvidia-smi and driver version"
