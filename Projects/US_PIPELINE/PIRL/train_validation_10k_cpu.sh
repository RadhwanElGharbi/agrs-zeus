#!/bin/bash
################################################################################
# US_PIPELINE PIRL - Validation Training (10K timesteps) - CPU VERSION
#
...
#
# Expected runtime (CPU, 24 envs): ~30-60 seconds
################################################################################
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}US_PIPELINE PIRL - Validation Training (CPU)${NC}"
echo -e "${BLUE}================================================${NC}"

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="${SCRIPT_DIR}/configs/us_pipeline_training_config.yaml"
OUTPUT_DIR="${SCRIPT_DIR}/outputs/validation_10k_cpu_$(date +%Y%m%d_%H%M%S)"
...
echo -e "${GREEN}Starting CPU validation training...${NC}"
echo -e "${YELLOW}⏱️  Expected runtime: ~30-60 seconds${NC}"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}✅ CPU Validation training complete!${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo ""
    ...
    echo "     $GEOJSON_OUTPUT"
    echo "  3. Run production training (CPU, 500K timesteps):"
    echo "     ./train_production_500k_cpu.sh"
    echo ""
else
    echo -e "${RED}========================================${NC}"
