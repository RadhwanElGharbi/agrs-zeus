...
Supports --quiet mode for minimal CLI output.
"""
...
class ProgressCallback(BaseCallback):
    ...
    """
    def __init__(self, check_freq: int = 1000, verbose: int = 1, quiet: bool = False):
    ...
    def _on_step(self) -> bool:
        if not self.quiet and self.n_calls % self.check_freq == 0:
...
class QuietEvalCallback(EvalCallback):
    """EvalCallback with reduced logging in quiet mode."""
    ...
    def _on_step(self) -> bool:
        ...
        # In quiet mode, only log when there's a new best
        if self.quiet and self.best_mean_reward > self._last_best_reward:
...
def create_vec_env(config_path: str, num_envs: int = 1, log_dir: str = None, quiet: bool = False):
    """
    ...
    Args:
        ...
        quiet: Enable quiet mode for reduced logging
    ...
    # Configure quiet mode for environments
...
def train_model(
    ...
    if not quiet:
        ...
        logger.info(f"🚀 Training {total_timesteps:,} timesteps | {num_envs} envs | quiet mode")
    ...
    model = PPO(
        ...
        vf_coef=0.5,
        max_grad_norm=0.5,
        verbose=0 if quiet else 1,  # Reduce SB3 verbosity in quiet mode
        device=device,
    ...
    # Configure logger (only CSV and tensorboard in quiet mode)
    if quiet:
        model_logger = configure(str(log_dir / "training"), ["csv", "tensorboard"])
    else:
    ...
    # 1. Evaluation callback (with quiet mode support)
    eval_callback = QuietEvalCallback(
        eval_env,
    ...
    # 2. Checkpoint callback (silent in quiet mode)
    ...
    # 3. Progress callback (with quiet mode)
    progress_callback = ProgressCallback(check_freq=1000, verbose=0 if quiet else 1, quiet=quiet)
    ...
    try:
        model.learn(
            ...
            log_interval=10 if not quiet else None,  # Disable log_interval in quiet mode
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Minimal output (progress bar only, batch episode summaries)"
    )
    ...
    if args.quiet:
        logging.getLogger('pirl_native_env_us').setLevel(logging.WARNING)
